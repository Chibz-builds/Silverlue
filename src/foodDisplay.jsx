      ...
      <div className="food-display-list">
        {food_list.map((item, index) => {
          if (category === "All" || category === item.category) {
            return <FoodItem key={index} id={item._id} name={item.name} description={item.description} price={item.sizes && item.sizes.micro > 0 ? item.sizes.micro : item.price} image={item.image} />
          }
        })}
      </div>
    </div>
